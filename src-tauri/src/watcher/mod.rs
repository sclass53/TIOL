...
impl FileWatcher {
    pub fn start(db: Arc<Db>, folders: Vec<(i64, String)>) -> Result<Self, String> {
            ...
            while let Some(changed_path) = rx.recv().await {
                ...
                if let Some((folder_path, folder_id)) = folder_opt {
                    ...
                    let handle = tokio::spawn(async move {
                        ...
                        if let Err(e) = crate::scanner::scan_folder(&db2, folder_id, &fp) {
                            log::error!("scan error {}: {}", fp, e);
                        }
                    });
    ...
    /// Add a new folder to watch at runtime (requires recreating watcher list externally; simplified: caller should restart watcher)
    pub fn notify_external_change(&self, path: String) {
        let _ = self.tx.try_send(path);
    }
}

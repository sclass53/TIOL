...
本地优先（local-first）的照片管理应用：照片全程留在你自己的硬盘上，AI 推理完全离线运行（SigLIP2 int8 嵌入 + 用户自定义零样本标签），不需要任何云端服务。

## 功能

- 📁 目录管理：添加/移除照片目录，文件系统监控（新增/修改自动入队处理）
- 🔍 双路径搜索：**语义搜索**（自然语言，如 "a cup of coffee"）+ **标签搜索**（毫秒级 SQL）
- 🏷️ 自定义标签：零样本 AI 打标——设置里输入任意标签（中英文均可），AI 自动匹配照片并给出置信度；支持多标签；未匹配照片标记为 `unknown` 防止重复检测
- 🖼️ 缩略图懒加载 + 缓存（500MB 上限自动清理），点击查看大图预览
- 🖥️ 推理后端可选：auto / GPU / CPU / Apple MLX（macOS 走 CoreML，Apple 原生加速）
- 🌐 中英双语界面；调试模式（应用内日志面板 + 推理进度浮窗）
